...
if sys.platform.startswith('win'):
    # Wrap stdout and stderr with UTF-8 encoding for Windows
...
class SafeStreamHandler(logging.StreamHandler):
    """Custom stream handler that handles Unicode encoding errors gracefully"""
    def emit(self, record):
        try:
            super().emit(record)
        except UnicodeEncodeError:
            # Fallback: encode with ASCII and replace problematic characters
            try:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('D:/Ethereal Glow/taqwin_file_watcher.log', encoding='utf-8'),
        SafeStreamHandler()
    ]
)
